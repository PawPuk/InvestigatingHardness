...
class EnsembleTrainer:
    ...
    def train_ensemble(self, train_loader: DataLoader, test_loader: DataLoader, training: str):
        ...
        if training == 'full':
            ...
            print(f'The class bias of trained ensemble is characterised by the following average class-level '
                  f'accuracies:\n\t{avg_class_accuracies}')
            u.save_data(avg_class_accuracies, f"{u.HARD_IMBALANCE_DIR}{self.dataset_name}"
                                              f"_avg_class_accuracies.pkl")

            # Find the hardest and easiest classes
            hardest_class = np.argmin(avg_class_accuracies)
            easiest_class = np.argmax(avg_class_accuracies)
            print(f"\nHardest class accuracy (class {hardest_class}): {avg_class_accuracies[hardest_class]*100:.3f}%")
            print(f"Easiest class accuracy (class {easiest_class}): {avg_class_accuracies[easiest_class]*100:.3f}%")
